    ...
    override val generators = generatorsBuilder {
        generatorFor(JavaLanguage) {
            ...
            current {
                ...
                val rectsVar = uniqueVariable("${rects.value.value}ByRatio", JavaTypes.ArrayList(JvmOpenCvTypes.MatOfPoint).new())
                ...
                current.scope {
                    ...
                    foreach(Variable(JvmOpenCvTypes.Rect, "rect"), rects.value) { contour ->
                        val ratioVar = uniqueVariable("ratio", contour.propertyValue("height", IntType).castTo(DoubleType) / contour.propertyValue("width", IntType).castTo(DoubleType))
                        local(ratioVar)

                        ifCondition((ratioVar greaterOrEqualThan (minRatioVar / 100.0.v)) and (ratioVar lessOrEqualThan (maxRatioVar / 100.0.v))) {
                            rectsVar("add", contour)
                        }
                    }
                }
        ...
        generatorFor(CPythonLanguage) {
            ...
            current {
                ...
                current.scope {
                    ...
                    foreach(Variable(CPythonLanguage.NoType, "rect"), rects.value) { contour ->
                        val ratioVar = uniqueVariable("area", (contour.propertyValue("width", IntType) / contour.propertyValue("height", IntType)))
                        local(ratioVar)

                        ifCondition((ratioVar greaterOrEqualThan  minRatioVal.value.v) and (ratioVar lessOrEqualThan maxRatioVal.value.v)) {
                            rectsVar("add", contour)
                        }
                    }
                }
